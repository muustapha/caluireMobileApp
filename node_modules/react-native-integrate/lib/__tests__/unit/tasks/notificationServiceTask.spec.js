...
const notificationServiceTask_1 = require("../../../tasks/notificationServiceTask");
...
describe('notificationServiceTask', () => {
    it('should skip insert when ifNotPresent exists', async () => {
        ...
        await (0, notificationServiceTask_1.notificationServiceTask)({
    ...
    it('should prepend text into didLaunchWithOptions', async () => {
        let content = notificationServiceM_1.notificationServiceM;
        const task = {
            task: 'notification_service',
            target: 'test',
            actions: [
        ...
        content = await (0, notificationServiceTask_1.notificationServiceTask)({
            configPath: 'path/to/config',
            task: task,
            content,
            packageName: 'test-package',
...
`);
    ...
    it('should append/prepend/insert after/before text into non existing blocks}', async () => {
        for (const block of [
            ...
            let content = notificationServiceM_1.notificationServiceM.replace(/(didReceiveNotificationRequest|serviceExtensionTimeWillExpire)/, 'foo');
            const task = {
                task: 'notification_service',
                target: 'test',
                actions: [
    ...
    it('should throw when block could not be added', async () => {
        const content = notificationServiceM_1.notificationServiceM.replace(/serviceExtensionTimeWillExpire/, 'foo');
    ...
    describe('runTask', () => {
        it('should read and write app delegate file', async () => {
            const notificationServicePath = (0, mockAll_1.writeMockNotificationService)();
            const task = {
                task: 'notification_service',
                target: 'test',
                actions: [
            ...
            await (0, notificationServiceTask_1.runTask)({
                configPath: 'path/to/config',
                task: task,
                packageName: 'test-package',
            });
        ...
        it('should throw when app delegate does not exist', async () => {
            ...
            await expect((0, notificationServiceTask_1.runTask)({
                configPath: 'path/to/config',
                task: task,
                packageName: 'test-package',
