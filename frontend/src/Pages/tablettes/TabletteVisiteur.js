  ...
  fetch('http://10.0.2.2:5127/api/Produits/magasin/tablette')
    ...
    return (
      <LinearGradient colors={['#ffffff', '#999999']} style={styles.container}>
        ...
        <Trier produits={tablettes} setProduits={setTablettes} />
<View style={styles.container1}>
  <View style={styles.container2}>
  {tablettes.map((item, index) => {
    return (
      <Produit key={index.toString()} item={item} navigation={navigation} produits={tablettes} setProduits={setTablettes} />
    );
  })}
</View>
</View>
