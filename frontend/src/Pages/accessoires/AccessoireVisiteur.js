...
const AccessoireVisiteur = ({ navigation }) => {
   ...
   useEffect(() => {
  fetch('http://10.0.2.2:5127/api/Produits/magasin/accessoire')
    .then(response => {
    ...
    return (
      <LinearGradient colors={['#ffffff', '#999999']} style={styles.container}>
        <Header icon={retour} title="ACCESSOIRES" navigation={navigation} />
 
        <Trier produits={accessoires} setProduits={setaccessoires} />
<View style={styles.container1}>
  <View style={styles.container2}>
  {accessoires.map((item, index) => {
    return (
      <Produit key={index.toString()} item={item} navigation={navigation} produits={accessoires} setProduits={setaccessoires} />
    );
  })}
</View>
</View>
