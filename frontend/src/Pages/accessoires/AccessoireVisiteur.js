...
const AccessoireVisiteur = ({ navigation }) => {
    const [Acessoire, setAcessoire] = useState([]);
    ...
    useEffect(() => {
      fetch('http://10.0.2.2:5127/api/Produits')
        ...
        .then(data => setAcessoire(data))
        .catch(error => console.error('Erreur:', error));
    }, []);

    useEffect(() => {
      let accessoireTries = [...Acessoire];
      if (filtre === 'marque') {
        acessoireTries.sort((a, b) => a.marque.localeCompare(b.marque));
      } else if (filtre === 'prix croissant') {
        acessoireTries.sort((a, b) => a.prix - b.prix);
      } else if (filtre === 'prix décroissant') {
       accessoireTries.sort((a, b) => b.prix - a.prix);
      }
      setAcessoire(accessoireTries);
    }, [filtre]);
    ...
    return (
      <LinearGradient colors={['#ffffff', '#999999']} style={styles.container}>
        <Header icon={retour} title={"Acessoire"} navigation={navigation}/>
        <View>
...
export default AccessoireVisiteur;